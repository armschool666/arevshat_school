  ...
  return (
    <SiteShell>
      ...
      <section className="section-wrap split">
        <div>
          <p className="eyebrow">{t("home.metaxeEyebrow")}</p>
          <h2>{t("home.metaxeTitle")}</h2>
          <p>{t("home.metaxeText")}</p>
          <Link href="/metaxe" className="btn-primary" style={{ display: "inline-block", marginTop: "1rem" }}>
            {t("home.metaxeReadMore")} →
          </Link>
        </div>
        <div>
          {/* eslint-disable-next-line @next/next/no-img-element */}
      ...
      {schoolConfig.reelUrl && (
        <section className="section-wrap">
          <div className="section-heading">
            <p className="eyebrow">{t("home.videoEyebrow")}</p>
            <h2>{t("home.videoTitle")}</h2>
          </div>
          <div style={{ position: "relative", width: "100%", paddingBottom: "56.25%", overflow: "hidden", borderRadius: "var(--radius, 8px)" }}>
            <iframe
              src={`https://www.facebook.com/plugins/video.php?href=${encodeURIComponent(schoolConfig.reelUrl)}&show_text=false`}
              style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "100%", border: "none" }}
              scrolling="no"
              allowFullScreen
              allow="autoplay; clipboard-write; encrypted-media; picture-in-picture; web-share"
              title={t("home.videoAriaLabel")}
              aria-label={t("home.videoAriaLabel")}
              loading="lazy"
            />
          </div>
        </section>
